...
@section('content')
    <section class=" ">
        <div class="card">
            <div class="card-header text-center">
                <h3> طرق الدفع المحذوفة : {{ count($payemntMethods) }}</h3>
            </div><!-- card-header -->
                   ...
                   @foreach ($payemntMethods as $payemntMethod)
                       ...
                       <tr>
                          <td>{{ $payemntMethods->FirstItem()+$loop->index}}</td>
                           ...
                           <td><a href="{{ route('payemnt-method.restore', ['id'=>$payemntMethod->id]) }}" class="btn btn-sm btn-success">ارجاع</a></td>
   ...
   <div class="modal fade" id="modal-default{{ $payemntMethod->id }}">
    <!-- start form -->
    <form action="{{ route('payemnt-methods.destroy', ['payemnt_method'=>$payemntMethod->id]) }}" method="post">
        @csrf
        @method('DELETE')
