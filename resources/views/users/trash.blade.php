    ...
    <section class=" ">
        <div class="card">
            <div class="card-header text-center">
                <h3> المطاعم المحذوفة : {{ count($users) }}</h3>
            </div><!-- card-header -->
                   ...
                   @foreach ($users as $user)
                       ...
                       <tr>
                          <td>{{ $users->FirstItem()+$loop->index}}</td>
                           ...
                           <td><a href="{{ route('user.restore', ['id'=>$user->id]) }}" class="btn btn-sm btn-success">ارجاع</a></td>

                           <td>

      <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#modal-default{{ $user->id }}"> حذف</button>
        ...
        <div class="modal-dialog">
          ...
          <div class="modal-content">
            ...
            <div class="modal-body text-danger">
              <p>هل انت متأكد هل تريد حذف البيانات : <span class="text-primary">{{ $user->name }}</span> <br>
